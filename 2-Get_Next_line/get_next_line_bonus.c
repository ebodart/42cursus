...
/*   Updated: 2021/02/25 17:24:09 by ebodart          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_next_line(int fd, char **line)
{
	...
	if (!line || (*line = NULL) || BUFFER_SIZE < 1 || fd < 0 || fd > FOPEN_MAX)
		return (-1);
	if (keep[fd] == NULL)
		if (!(keep[fd] = ft_calloc(BUFFER_SIZE + 1, sizeof(char))))
			return (-1);
	if (*keep[fd])
	...
	while (ret_read > 1)
		ret_read = next_line(keep[fd], fd, line);
	if (ret_read < 1)
	{
		free(keep[fd]);
		keep[fd] = NULL;
